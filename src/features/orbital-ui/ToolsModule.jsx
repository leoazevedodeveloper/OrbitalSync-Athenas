...
import { Mic, MicOff, Settings, Power, Video, VideoOff, Hand, CalendarDays } from 'lucide-react';
...
const ToolsModule = ({
    ...
    return (
        <div
            ...
            className={`fixed z-[38] pointer-events-none transition-opacity duration-300 ${_activeDragElement === 'tools' ? 'opacity-100' : ''
                }`}
            style={{
                left: position.x,
                bottom: position.y,
        ...
        >
            <div
                className={`pointer-events-auto flex flex-row items-center gap-0.5 rounded-full border border-white/[0.08] bg-zinc-950/35 px-1 py-1 shadow-[0_4px_24px_rgba(0,0,0,0.45),inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-2xl ${_activeDragElement === 'tools' ? 'ring-1 ring-white/12' : ''
                    }`}
            >
                <button
                    type="button"
                    ...
                    className={`${base} ${isConnected
                        ? 'text-emerald-300/95 hover:bg-emerald-400/[0.1]'
                        : idle
                        }`}
                >
                    <span
                        className={`pointer-events-none absolute inset-0 rounded-full opacity-0 transition-opacity duration-300 ${isConnected ? 'opacity-100 bg-emerald-400/[0.08]' : ''
                            }`}
                    />
                ...
                <button
                    ...
                    className={`${base} ${isHandTrackingEnabled
